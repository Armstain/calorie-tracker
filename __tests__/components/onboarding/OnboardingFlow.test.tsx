...
jest.mock('@/components/onboarding/WelcomeStep', () => {
  return function MockWelcomeStep({ onContinue, onSkip }: { onContinue: () => void; onSkip: () => void }) {
    return (
      <div data-testid="welcome-step">
        <button onClick={onContinue} data-testid="continue-btn">Continue</button>
...
jest.mock('@/components/onboarding/BasicInfoStep', () => {
  return function MockBasicInfoStep({ onContinue, onBack }: { onContinue: (data: any) => void; onBack: () => void }) {
    return (
      <div data-testid="basic-info-step">
        <button onClick={() => onContinue({ age: 25, gender: 'male' })} data-testid="continue-btn">Continue</button>
...
jest.mock('@/components/onboarding/ActivityStep', () => {
  return function MockActivityStep({ onContinue, onBack }: { onContinue: (data: any) => void; onBack: () => void }) {
    return (
      <div data-testid="activity-step">
        <button onClick={() => onContinue({ level: 'moderate', exerciseFrequency: 3 })} data-testid="continue-btn">Continue</button>
        <button onClick={onBack} data-testid="back-btn">Back</button>
      </div>
    );
  };
...
jest.mock('@/components/onboarding/GoalsStep', () => {
  return function MockGoalsStep({ onContinue, onBack }: { onContinue: (data: any) => void; onBack: () => void }) {
    return (
      <div data-testid="goals-step">
        <button onClick={() => onContinue({ primary: 'weight_loss', targetCalories: 1800 })} data-testid="continue-btn">Continue</button>
        <button onClick={onBack} data-testid="back-btn">Back</button>
      </div>
    );
  };
...
jest.mock('@/components/onboarding/CompletionStep', () => {
  return function MockCompletionStep({ onComplete, onBack }: { onComplete: () => void; onBack: () => void }) {
    return (
...
describe('OnboardingFlow', () => {
  ...
  it('progresses through all steps when continuing', async () => {
    ...
    expect(mockOnComplete).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        activity: expect.objectContaining({ level: 'moderate', exerciseFrequency: 3 }),
        goals: expect.objectContaining({ primary: 'weight_loss', targetCalories: 1800 })
      })
    );
  });
  ...
  it('initializes with provided initial profile data', () => {
    ...
    render(<OnboardingFlow onComplete={mockOnComplete} initialProfile={initialProfile} />);
